        ...
        switch (engine) {
          ...
          case 'msq-viper': {
            if (cancelQueryId) {
              void cancelToken.promise
                .then(cancel => {
                  if (cancel.message === QueryManager.TERMINATION_MESSAGE) return;
                  return Api.instance.delete(
                    `/druid/v2/sql/viper/${Api.encodePath(cancelQueryId)}`,
                  );
                })
                .catch(() => {});
            }
            ...
            const executionPromise = Api.instance
              ...
              .then(
                ({ data: viperResponse }) => {
                  if (deepGet(query, 'context.fullReport')) {
                    const viperReport = viperResponse[viperResponse.length - 1][0];

                    return Execution.fromTaskReport(viperReport)
                      .changeEngine('msq-viper')
