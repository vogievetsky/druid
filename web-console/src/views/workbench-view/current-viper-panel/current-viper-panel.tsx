...
import { ViperDetailsDialog } from '../viper-details-dialog/viper-details-dialog';
import { workStateStore } from '../work-state-store';

import './current-viper-panel.scss';

interface CurrentViperEntry {
  sqlQueryId: string;
  sql: string;
  identity: string;
  startTime: string;
...
export const CurrentViperPanel = React.memo(function CurrentViberPanel(
  ...
  const [currentViperState, queryManager] = useQueryManager<number, CurrentViperEntry[]>({
    query: workStateVersion,
    processQuery: async _ => {
      return (await Api.instance.get('/druid/v2/sql/viper')).data.queries;
    },
  });
  ...
  const currentViper = currentViperState.getSomeData();
  return (
    <div className="current-viper-panel">
      <div className="title">
        Current MSQ-interactive queries
      ...
      {confirmCancelId && (
        <CancelQueryDialog
          ...
          onCancel={async () => {
            ...
            try {
              await Api.instance.delete(`/druid/v2/sql/viper/${Api.encodePath(confirmCancelId)}`);
      ...
      {showSql && <ViperDetailsDialog sql={showSql} onClose={() => setShowSql(undefined)} />}
    </div>
  );
});
